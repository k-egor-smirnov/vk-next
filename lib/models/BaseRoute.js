class BaseRoute {
  ...
  _process(ctx, message) {
    ...
    Object.keys(this.commands).map(key => {
      ...
      if (
        typeof ctx.message.body === "string" &&
        ctx.message.body.startsWith(key)
      ) {
        func = this[this.commands[key]];
      }
    });
  ...
  _processQuestion(ctx, question, message) {
    if (typeof question !== "object")
      throw new Error("Question validator not provided");
    if (message.text === question.data.text && message.peer_id === ctx.userId)
      return;
    if (message.body === question.data.text && message.user_id === ctx.userId)
      return;
